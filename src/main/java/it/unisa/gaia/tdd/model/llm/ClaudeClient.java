...
public class ClaudeClient implements LLMClient {
    private static final Logger LOGGER = Logger.getInstance(ClaudeClient.class);
    ...
    private String callClaude(String system, String user) throws IOException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                throw new IOException("Anthropic Error: " + response.code() + " " + (response.body() != null ? response.body().string() : ""));
            }
            JsonNode root = mapper.readTree(response.body().string());

            // Claude response structure: { "content": [ { "text": "...", "type": "text" } ] }
            if (root.has("content") && root.get("content").isArray() && root.get("content").size() > 0) {
                String content = root.path("content").get(0).path("text").asText();
                return extractCode(content);
            } else {
                throw new IOException("Unexpected response format from Anthropic");
            }
        }
    }
